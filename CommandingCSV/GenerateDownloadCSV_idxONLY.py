...
for line in lines:
    ...
    # The first column is the file path (e.g., "/sd/padreMDA0_250401212029.idx")
    parts = line.split(",")
    file_path = parts[0]
    ...
    # Check if the file name starts with "padreMD" or "padreSP", and ends with ".idx"
    if (lower_name.startswith("padremd") or lower_name.startswith("padresp")) and lower_name.endswith(".idx"):
        # Expected format: padre####_YYMMDDHHMMSS.idx
        ...
        if underscore_index != -1 and dot_index != -1:
            ...
            if len(time_str) >= 10:
                ...
                if abs(file_dt - dt_pull) <= timedelta(minutes=10):
                    # Convert the file name (base_name) to uppercase and store it.
                    selected_files.append(base_name.upper())

# ----------------------------
# Write the output CSV without extra empty rows
# ----------------------------
